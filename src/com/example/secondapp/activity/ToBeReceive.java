...
public class ToBeReceive extends BaseActivity implements OnClickListener{
	Button btn1;
	Button btn2;
	LinearLayout back;
	...
	protected void onCreate(Bundle arg0) {
		...
		super.onCreate(arg0);
		setContentView(R.layout.tobereceive);
	...
	public void onClick(View v) {
		switch (v.getId()) {
		case R.id.btn1:
			Intent intent = new Intent(ToBeReceive.this, ViewLogistics.class);
	...
	public void showMyDialog(final int position) {
		...
		dialog.setNegativeButton("确定", new DialogInterface.OnClickListener() {
			...
			public void onClick(DialogInterface dialog, int which) {
				...
				MyOrderAdapter2.list.remove(position);
				MOFragment2.adapter.notifyDataSetChanged();
				if (MyOrderAdapter2.list.size() == 0) {
					MOFragment2.noorder.setVisibility(View.VISIBLE);
				}
				finish();
			}
		});
